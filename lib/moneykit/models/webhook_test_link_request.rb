...
module MoneyKit
  class WebhookTestLinkRequest
    # The webhook event you want to trigger for a link.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MoneyKit::WebhookTestLinkRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MoneyKit::WebhookTestLinkRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] webhook_event Value to be assigned
    def webhook_event=(webhook_event)
      if webhook_event.nil?
        fail ArgumentError, 'webhook_event cannot be nil'
      end
    ...
    def hash
      [webhook_event, webhook_idempotency_key].hash
    end
