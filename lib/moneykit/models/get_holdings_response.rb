...
module MoneyKit
  class GetHoldingsResponse
    # The holdings belonging to investment accounts for this link.
    attr_accessor :holdings

    # The set of securities for these holdings.
    attr_accessor :securities

    # The set of accounts for which holdings are returned.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MoneyKit::GetHoldingsResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MoneyKit::GetHoldingsResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] holdings Value to be assigned
    def holdings=(holdings)
      if holdings.nil?
        fail ArgumentError, 'holdings cannot be nil'
      end
    ...
    def hash
      [holdings, securities, accounts, link].hash
    end
