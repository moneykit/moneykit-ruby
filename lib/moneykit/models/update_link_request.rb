...
module MoneyKit
  class UpdateLinkRequest
    ...
    # Arbitrary strings used to describe this link. Tags must be under 100 characters in length, and begin with an alphanumeric character.
    attr_accessor :link_tags

    # DEPRECATED; please use `link_tags` instead.
    attr_accessor :tags
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MoneyKit::UpdateLinkRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
