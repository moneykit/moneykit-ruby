...
module MoneyKit
  class TransactionSync
    # A list of transactions that have been added to the link.
    attr_accessor :created

    # A list of transactions that have been updated on the link.
    attr_accessor :updated

    # A list of transaction ids that have been removed from the link.
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MoneyKit::TransactionSync`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
