...
module MoneyKit
  class GetUserAccountsResponse
    # The set of all accounts belonging to this user,         as a dictionary of `{link_id:[accounts]}`.
    attr_accessor :links
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MoneyKit::GetUserAccountsResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
