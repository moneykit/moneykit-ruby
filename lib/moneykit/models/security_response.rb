...
module MoneyKit
  class SecurityResponse
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MoneyKit::SecurityResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] security_id Value to be assigned
    def security_id=(security_id)
      if security_id.nil?
        fail ArgumentError, 'security_id cannot be nil'
      end
