...
module MoneyKit
  class CustomerUserPhone
    # The user's phone number, preferably in E164 format, including the country code.
    attr_accessor :number

    # Optional two-letter country code, which can be supplied when the phone number does not start             with the E164 numeric country code.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MoneyKit::CustomerUserPhone` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MoneyKit::CustomerUserPhone`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
