...
module MoneyKit
  class Owner
    ...
    # A list of addresses for the account owner.         <p>MoneyKit does not (yet) perform address verification or normalization on this data, so it may contain         inaccurate or partial address information, and it may not appear be in a country-specific canonical format.
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MoneyKit::Owner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [names, addresses, phone_numbers, emails].hash
    end
